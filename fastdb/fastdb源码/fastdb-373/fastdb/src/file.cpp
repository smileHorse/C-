...
{ 
	printf("getMaxPages:%d,%d,%d\n",dbDatabaseOffsetBits,dbModMapBlockBits,1 << (dbDatabaseOffsetBits - dbModMapBlockBits));
...
printf("open-8-end-2\n");
    if (replicationSupport) { 
		...
		printf("open-8-end-2-1\n");
#ifndef DISKLESS_CONFIGURATION
		printf("open-8-end-2-1-1\n");
        cfd = ::open(cFileName, O_RDWR|O_DSYNC|O_CREAT, 0666);
        delete[] cFileName;
        if (cfd < 0) { 
            return errno;
        }
		printf("open-8-end-2-1-2\n");
        if (ftruncate(cfd, nPages*sizeof(int)) != ok) {
            status = errno;
            ::close(cfd);
            return status;      
        }
		printf("open-8-end-2-1-3\n");
#else
		printf("open-8-end-1\n");
#ifndef MAP_ANONYMOUS 
