...
{
	//printf("fastdb-dbHashTable::find-1\n");
    dbHashTable* hash = (dbHashTable*)db->get(hashId);
	//printf("fastdb-dbHashTable::find-2\n");
    if (hash->size == 0) { 
        return;
    }
	//printf("fastdb-dbHashTable::find-3\n");
    if (sc.field->_hashcode == NULL) { 
        sc.field->_hashcode = getHashFunction(db->getVersion(), sc.field);
    }
	//printf("fastdb-dbHashTable::find-4:%d\n",sc.type);
    unsigned keylen;
    unsigned hashcode;
    switch (sc.type) {
      case dbField::tpString:
	...
	//printf("fastdb-dbHashTable::find-5\n");
    unsigned h = hashcode % hash->size;
    oid_t itemId = 
        ((oid_t*)db->get(hash->page + h/dbIdsPerPage))[h % dbIdsPerPage];
    dbTable* table = (dbTable*)db->getRow(sc.cursor->table->tableId);
	//printf("fastdb-dbHashTable::find-6\n");

    while (itemId != 0 ) { 
		//printf("fastdb-dbHashTable::find-6-0:%d\n",itemId);
        dbHashTableItem* item = (dbHashTableItem*)db->get(itemId);
		//printf("fastdb-dbHashTable::find-6-1\n");
        sc.probes += 1;
        if (item->hash == hashcode) { 
			//printf("fastdb-dbHashTable::find-6-2\n");
            byte* rec = (byte*)db->getRow(item->record);
			//printf("fastdb-dbHashTable::find-6-3:%d\n",sc.type);
            bool equals;
            switch (sc.type) { 
              case dbField::tpString:
              case dbField::tpWString:
			...
			//printf("fastdb-dbHashTable::find-6-4:\n");
            if (equals) { 
                if (!sc.condition 
	...
	//printf("fastdb-dbHashTable::find-7\n");
}
