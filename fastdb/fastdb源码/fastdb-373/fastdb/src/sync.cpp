...
bool dbSharedMemory::open(char const* name, size_t size)
{
	...
	printf("dbSharedMemory::open-4\n");
    if (key < 0) { 
        return false;
    }
	printf("dbSharedMemory::open-5\n");
    shm = shmget(key, DOALIGN(size, 4096), IPC_CREAT|ACCESS_PERMISSION_MASK);
    if (shm < 0) { 
        return false;
    }
	printf("dbSharedMemory::open-6\n");
