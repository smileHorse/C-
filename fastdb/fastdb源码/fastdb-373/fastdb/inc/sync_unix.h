...
class dbSharedObject { 
  ...
  public:
    ...
    bool open(char* fileName) { 
		printf("dbSharedObject : open\n");
        delete[] name;
        name = new char[strlen(fileName) + 1];
        strcpy(name, fileName);
        fd = ::open(fileName, O_RDWR|O_CREAT, 0777);
		printf("dbSharedObject : open-1\n");
        if (fd < 0) { 
            return false;
        }
		printf("dbSharedObject : open-2\n");
        if (ftruncate(fd, sizeof(T)) < 0) {
            ::close(fd);
            return false;
        }
		printf("dbSharedObject : open-3\n");
        ptr = (T*)mmap(NULL,
		...
		printf("dbSharedObject : open-4\n");
        if (ptr == MAP_FAILED) { 
            ptr = NULL;
            ::close(fd);
            return false;
        }
		printf("dbSharedObject : open-5\n");
        return true;
    }
...
class dbSharedObject : public dbSharedMemory { 
  public:
    bool open(char* name) { 
		printf("dbSharedObject : public dbSharedMemory\n");
